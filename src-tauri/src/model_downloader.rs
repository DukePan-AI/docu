...
pub fn seed_bundled_models(resource_dir: &Path, models_dir: &Path) {
    ...
    // 임베딩 모델도 설치본에 포함한다. 회사망에서 설치 후 시맨틱 검색이
    // Hugging Face에 다시 접근하지 않도록 AppData의 기존 모델만 보충한다.
    let embed_dest_dir = models_dir.join("kosimcse-roberta-multitask");
    if let Err(e) = fs::create_dir_all(&embed_dest_dir) {
        tracing::warn!("seed: 임베딩 모델 디렉토리 생성 실패: {}", e);
    } else {
        seed_one(
            &bundled_root
                .join("models")
            ...
            &embed_dest_dir.join("model_int8.onnx"),
            KOSIMCSE_MODEL_SHA256,
            "KoSimCSE 임베딩 모델",
        );
        seed_one(
            &bundled_root
                ...
                .join("kosimcse-roberta-multitask")
                .join("tokenizer.json"),
            &embed_dest_dir.join("tokenizer.json"),
            KOSIMCSE_TOKENIZER_SHA256,
            "KoSimCSE 토크나이저",
        );
    }
