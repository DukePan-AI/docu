...
export function HelpModal({ isOpen, onClose, onRestartTour, initialSection = "start" }: HelpModalProps) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Finder 사용 가이드">
...
function StartSection() {
  return (
    <div>
      <SectionTitle>Finder에 오신 것을 환영합니다!</SectionTitle>
      <Paragraph>
        Finder는 PC에 저장된 문서를 빠르게 찾아주는 검색 앱이에요. 한글(HWPX), 워드(DOCX), 파워포인트(PPTX), 엑셀(XLSX), PDF, TXT 파일은 물론 이미지(OCR)까지 검색할 수 있어요.
      </Paragraph>
...
function SearchSection() {
  return (
    <div>
      ...
      <SubTitle>Finder에게 질문 (AI 답변)</SubTitle>
      <Paragraph>
        같은 토글에서 'Finder' 모드를 선택하면 문서에 대해 질문할 수 있어요. "연차 사용 조건이 어떻게 되나요?"처럼 입력하고 Enter를 누르면 관련 문서를 찾아 내용을 근거로 답변해줘요. 설정 → AI 탭에서 API 키를 등록해야 동작해요.
      </Paragraph>
