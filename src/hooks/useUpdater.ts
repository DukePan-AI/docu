...
const GITHUB_REPO = "DukePan-AI/docu";
// latest.json은 저장소의 최신 릴리스 하나만 가리키므로, 다른 flavor/앱의 릴리스가
// 섞여도 설치하지 않도록 릴리스 본문에 앱 전용 채널 마커를 요구한다.
const UPDATE_CHANNEL_MARKER = "FINDER_UPDATE_CHANNEL=windows-online";
...
export function useUpdater(auto: boolean = true) {
  ...
  const checkForUpdate = useCallback(async (): Promise<Update | null> => {
    ...
    try {
      const u = await check();
      if (u) {
        // 마커가 없는 릴리스는 버전이 더 높아도 이 앱의 배포본으로 인정하지 않는다.
        // 기존에 잘 동작하던 UI/기능이 다른 앱 릴리스로 교체되는 것을 방지한다.
        if (!u.body?.includes(UPDATE_CHANNEL_MARKER)) {
        ...
        updateRef.current = u;
        const releaseNotes = u.body?.replace(UPDATE_CHANNEL_MARKER, "").trim();
        setState((s) => ({
          ...s,
          phase: "available",
          version: u.version,
